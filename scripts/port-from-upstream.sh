...
Usage: ./scripts/port-from-upstream.sh [upstream-ref] [live-source-ref]
  ...
  live-source-ref  Live tree to copy features from (default: v1.0.5 if tagged,
...
if [[ -z "$LIVE_ARG" ]]; then
  # Prefer v1.0.5 (expanding-window fill + hardened cue cache), else v1.0.4, else newest v* tag, else origin/main.
  if git rev-parse --verify -q "v1.0.5" >/dev/null 2>&1 || \
     git rev-parse --verify -q "refs/tags/v1.0.5" >/dev/null 2>&1; then
    LIVE_ARG="v1.0.5"
  elif git rev-parse --verify -q "v1.0.4" >/dev/null 2>&1 || \
